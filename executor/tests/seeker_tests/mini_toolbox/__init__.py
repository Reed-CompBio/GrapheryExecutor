...
def create_temp_folder(
    ...
    temp_folder = pathlib.Path(
        tempfile.mkdtemp(prefix=prefix, suffix=suffix, dir=parent_folder)
    )
    try:
...
class TempValueSetter(object):
    ...
    def __init__(self, variable, value, assert_no_fiddling=True):
        ...
        if (
            hasattr(first, "__getitem__")
            and hasattr(first, "get")
            and hasattr(first, "__setitem__")
            and hasattr(first, "__delitem__")
        ):
            # `first` is a dictoid; so we were probably handed a `(dict, key)`
            # pair.
            self.getter = lambda: first.get(second, NotInDict)
            self.setter = lambda value: (
                first.__setitem__(second, value)
                if value is not NotInDict
                else first.__delitem__(second)
            )
...
class OutputCapturer(object):
    ...
    def __init__(self, stdout=True, stderr=True):
        ...
        if stdout:
            self._stdout_temp_setter = TempValueSetter((sys, "stdout"), self.string_io)
        else:  # not stdout
            self._stdout_temp_setter = BlankContextManager()

        if stderr:
            self._stderr_temp_setter = TempValueSetter((sys, "stderr"), self.string_io)
        else:  # not stderr
    ...
    output = property(
        lambda self: self.string_io.getvalue(),
