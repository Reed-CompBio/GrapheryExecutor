...
class _BaseEntry(ABC):
    ...
    def __repr__(self):
        ...
        attributes = {
            ...
            for key in init_arguments
            if getattr(self, key) is not None
        }
        return "%s(%s)" % (
            type(self).__name__,
            ", ".join(
                "{key}={value}".format(**locals()) for key, value in attributes.items()
            ),
        )
...
class _BaseValueEntry(_BaseEntry):
    ...
    def check(self, s):
        ...
        return self._check_preamble(preamble) and self._check_content(content)
...
class CallEndedByExceptionEntry(_BaseEntry):
    ...
    def check(self, s):
        return re.match(r"""(?P<indent>(?: {4})*)Call ended by exception""", s)
...
class VariableEntry(_BaseValueEntry):
    def __init__(
        ...
        self.value = value
        self.stage = stage
        self.name_regex = None if name_regex is None else re.compile(name_regex)
        self.value_regex = None if value_regex is None else re.compile(value_regex)

    _preamble_pattern = re.compile(r"""^(?P<stage>New|Modified|Starting) var$""")
    ...
    _content_pattern = re.compile(r"""^(?P<name>.+?) = (?P<value>.+)$""")
...
class ReturnValueEntry(_BaseSimpleValueEntry):
    _preamble_pattern = re.compile(r"""^Return value$""")


class ExceptionValueEntry(_BaseSimpleValueEntry):
    _preamble_pattern = re.compile(r"""^Exception$""")
...
class SourcePathEntry(_BaseValueEntry):
    def __init__(self, source_path=None, source_path_regex=None, prefix=""):
        ...
        self.source_path_regex = (
            None if source_path_regex is None else re.compile(source_path_regex)
        )

    _preamble_pattern = re.compile(r"""^Source path$""")
...
class _BaseEventEntry(_BaseEntry):
    def __init__(
        ...
        self.source = source
        self.source_regex = None if source_regex is None else re.compile(source_regex)
        self.thread_info = thread_info
        self.thread_info_regex = (
            None if thread_info_regex is None else re.compile(thread_info_regex)
        )
    ...
    def check(self, s):
        ...
        return (
            ...
            and self._check_source(source)
            and self._check_thread_info(thread_info)
        )
