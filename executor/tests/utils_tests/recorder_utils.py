...
class ChangeList:
    ...
    def record(self, line_no: int, variables: dict = None, accesses: list = None):
        self.change_list.append(
            {"line": line_no, "variables": variables, "accesses": accesses}
        )
        return self

    @staticmethod
    def gen_record(line_no: int, variables: dict = None, accesses: list = None):
        return {"line": line_no, "variables": variables, "accesses": accesses}
    ...
    def loop_records(self, loop_num: int, *records: Tuple):
        for index in range(loop_num):
            for record in records:
                self.record(
                    record[0],
                    ChangeList.replace_values_with_index(index, record[1])
                    if len(record) > 1
                    else None,
                    ChangeList.replace_items_with_index(index, record[2])
                    if len(record) > 2
                    else None,
                )
        return self
