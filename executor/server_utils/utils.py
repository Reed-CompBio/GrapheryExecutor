...
from .params import (
    DEFAULT_PORT,
    GRAPH_OBJ_ANCHOR_NAME,
    ENTRY_PY_MODULE_NAME,
    MAIN_FUNCTION_NAME,
    ENTRY_PY_FILE_NAME,
    DEFAULT_SERVE_URL,
)
...
def execute(
    ...
    with controller as folder_creator, folder_creator(
        folder_hash, auto_delete=auto_delete_cache
    ) as cache_folder, TempSysPathAdder(cache_folder):
        try:
            entry_file: pathlib.Path = (
                cache_folder.cache_folder_path / ENTRY_PY_FILE_NAME
            )

            entry_file.write_text(code)
        except Exception as e:
            raise ExecutionServerException(
                f"Cannot create temporary execution file. Error: {e}"
            )
        ...
        try:
            ...
            controller.tracer_cls.log_output(
                _SOURCE_CODE_LOGGING_TEMPLATE.format(code_string=source_code)
            )
        ...
        try:
            ...
            if not main_function or not isinstance(main_function, Callable):
                raise ExecutionServerException(
                    "There is no main function or it is not valid"
                )

            if not hasattr(imported_module, GRAPH_OBJ_ANCHOR_NAME):
                raise ExecutionServerException(
                    "There is no graph object, which violates the naming convention"
                )
            ...
            raise ExecutionException(
                ...
                (
                    ...
                    if tb.filename.endswith(ENTRY_PY_FILE_NAME)
                ),
            )
        finally:
