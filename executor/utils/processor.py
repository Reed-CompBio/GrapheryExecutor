...
class Processor:
    ...
    def load_variables_and_create_color_map(
        self, variables: Set[Tuple[str, str]]
    ) -> None:
        if len(variables) > self.variable_number_limit and not self._no_limit:
            raise AssertionError(
                "The number of variable input cannot exceed {}!".format(
                    self.variable_number_limit
                )
            )
    ...
    def load_data(
        self, change_list: List[Mapping], variables: Set[Tuple[str, str]]
    ) -> object:
        ...
        record_mapping.update(
            zip([identifier_to_name(var) for var in variables], [None] * len(variables))
        )
    ...
    def generate_record_template(
        ...
        if isinstance(variable_changes, Mapping):
            ...
            for key, value in variable_changes.items():
                record_mapping[identifier_to_name(key)] = self.resolve_variable(
                    key, value
                )

        record_mapping[self.ACCESSED_ID_NAME] = self.resolve_accessed(
    ...
    def resolve_variable(
        self, name: Tuple[str, str], value: Any
    ) -> Union[str, Mapping]:
        ...
        if isinstance(value, (Node, Edge)):
            ...
            variable_value = self.process_graph_elements(value, name)
        else:
            variable_value = self.process_normal_variables(value)

        return variable_value

    def process_graph_elements(
        self, element: Union[Node, Edge], name: Tuple[str, str]
    ) -> dict:
        return {
            "type": "graph_element",
            "id": element.content,
    ...
    @staticmethod
    def process_normal_variables(element: Any) -> dict:
        return {"type": "normal_var", "repr": repr(element)}
